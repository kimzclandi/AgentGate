...
from state_evidence import score_state
...
def score(case, raw):
    ...
    result = dict(id=case['id'], tool_selection_exact=selected, parameters_exact=parameters,
                  ...
                  write_state_correct=score_state(case, raw),
                  write_state_note='Null means no state observation. Values cover seeded ticket request boundaries, not continuous or concurrent writes.')
    failures = []
    for key, label in [('tool_selection_exact', 'tool_selection'), ('parameters_exact', 'tool_arguments'),
                       ...
                       ('final_required_quotes_present', 'answer_quote'), ('followup_no_tools', 'followup_tool_constraint'), ('write_state_correct', 'database_state')]:
...
def read_protocol(folder):
    ...
    if protocol.get('state_evidence_schema') not in (None, 'ticket-state-v1'):
        raise ValueError('Unsupported state evidence schema')
...
def evaluate(folder):
    ...
    for case in cases:
        ...
        if protocol.get('state_evidence_schema') and raw.get('state_evidence') is None:
            raise ValueError('Required state observations missing')
        rows.append(score(case, raw))
    metrics = {}
    ...
    return dict(scorer_version='trajectory-score-v4', execution_mode='saved_output_replay',
                ...
                cases=rows, metrics=metrics, limitations=[
                    'HTTP stop alone is not proof of no database write; state observations cover seeded request boundaries only.',
...
def main():
    ...
    result['receipt'] = dict(created_utc=datetime.now(timezone.utc).isoformat(),
        ...
        state_scorer_sha256=hashlib.sha256((Path(__file__).parent/'state_evidence.py').read_bytes()).hexdigest())
