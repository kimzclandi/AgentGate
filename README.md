...
- **Agent 任务执行**：支持文档读取、工单查询与更新；提供确定性本地模式和可配置的 Chat Completions 兼容模型接口。
...
默认控制台使用确定性本地任务模式，不调用外部模型。远端模型入口为 `POST /api/agent/remote`，请求格式为 `{"task":"读取 doc-1 文档"}`，需要身份凭证并显式配置：
...
当前为单实例、受信任内置工具执行架构。支持可选模型和 OIDC 适配，但尚未完成真实模型提供商及 IdP 的外部集成验证。尚不支持任意代码执行、向量 RAG、远端业务写入对账或多实例协调。
