...
审批等待计入运行 300 秒有效期。超时或服务重启后，旧对话仅供查看，需要新建任务；已提交的写入不会因为最终回答失败而撤销。
...
最新原始输出：[local-model-results.json](local-model-results.json)。本机 Apple M4，Ollama 报告系统内存 16 GiB；CPU 推理。模型 digest：`8f68893c685c3ddff2aa3fffce2aa60a30bb2da65ca488b61fff134a4d1730e7`。首个冷启动文档读取约 3.85 秒；这不是吞吐基准或 SLA。精确复现仍可能受模型版本、采样、已有上下文和平台影响。
